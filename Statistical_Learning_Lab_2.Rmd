...
```{r}
temp <- as.data.frame(temp)
ggplot(temp,aes( y = avg,x=factor(k_sh_p))) +
  geom_bar(stat="identity",position="dodge") +
  facet_grid(~factor(k_col_sig))+
  ...
  facet_grid(~factor(k_col_sig))+ theme_light()  +
...
```
```{r}
ggplot(temp2, aes(y = run.time, x =factor(k_sh_p))) + facet_grid(~factor(k_col_sig))+
